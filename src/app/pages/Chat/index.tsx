    ...
    *  For responsive view */
    ...
    private setRightMenu = (force?: boolean) => {
        ...
        setTimeout(() => {
            ...
            this.messageComponent.animateToEnd(true);
            if (!rightMenu) {
                this.setState({
                    rightMenu: false,
                });
    ...
    private updateMessageDeleteHandler = (data: UpdateMessagesDeleted.AsObject) => {
        ...
        this.messageRepo.removeMany(data.messageidsList).then(() => {
            ...
            if (this.state.selectedDialogId === (peer.id || '')) {
    ...
    private getMessagesByDialogId(dialogId: string, force?: boolean, messageId?: string) {
        ...
        this.messageRepo.getMany({peer, limit: 40, before, ignoreMax: true}, (data) => {
            ...
            this.setState({
                ...
                if (dialog) {
                    this.messageComponent.setTopMessage(dialog.topmessageid || 0);
                }
                this.messageComponent.cache.clearAll();
                this.messageComponent.setMessages(dataMsg.msgs, () => {
                    if (force === true) {
    ...
    private messageLoadMoreBeforeHandler = () => {
        ...
        this.messageRepo.getMany({
            ...
            // Checks peerid on transition
            if (this.state.selectedDialogId !== dialogId || data.length === 0 || !this.messageComponent) {
                this.setLoading(false);
                return;
            }
            this.messageComponent.takeSnapshot();
    ...
    private getDialogById(id: string): IDialog | null {
        if (this.state.dialogMap.hasOwnProperty(id)) {
            return this.state.dialogs[this.state.dialogMap[id]] || null;
        }
        return null;
    }
    ...
    private updateDialogs(msg: IMessage, accessHash: string, force?: boolean) {
        ...
        if (this.state.dialogMap.hasOwnProperty(id)) {
            ...
            dialogs.push(dialog);
            const {dialogMap} = this.state;
            dialogMap[id] = dialogs.length - 1;
            this.setState({
                dialogMap,
            });
        }
    ...
    private updateDialogsNotifySettings(peerId: string, settings: PeerNotifySettings.AsObject) {
        ...
        if (this.state.dialogMap.hasOwnProperty(peerId)) {
            const index = this.state.dialogMap[peerId];
        ...
        this.dialogRepo.getManyCache({}).then((res) => {
            this.dialogsSort(res, (dialogs) => {
                data.ids.forEach((id: string) => {
                    ...
                    if (this.state.dialogMap.hasOwnProperty(id) && dialogs[this.state.dialogMap[id]]) {
                        window.console.debug('dialogDBUpdated peerId:', dialogs[this.state.dialogMap[id]].peerid);
                        const maxReadInbox = dialogs[this.state.dialogMap[id]].readinboxmaxid || 0;
        ...
        if (messages) {
            if (messages[info.stopIndex]) {
                ...
                if (messages[info.stopIndex].id !== -1) {
                    ...
                    this.sendReadHistory(this.state.peer, Math.floor(messages[info.stopIndex].id || 0));
                }
            }
        }
    }
    ...
    private messageJumpToMessageHandler = (id: number, text?: string) => {
        ...
        if (index > 0) {
            ...
            this.messageRepo.getMany({peer, after: id - 1, limit: 25}).then((res) => {
                if (this.state.selectedDialogId !== dialogId || res.length === 0 || !this.messageComponent) {
                    this.setLoading(false);
                    return;
                }
                ...
                setTimeout(() => {
                    ...
                    if (this.messageComponent) {
                        this.messageComponent.tryLoadBefore();
                    }
                }, 100);
            }).catch((err) => {
                this.setLoading(false);
